    ...
    <div class="CreateContainer">
        <form method="POST" @submit.prevent="CreatePost">
            <div v-if="step === 0">
                ...
                <input type="text" id="titleInput" name="titleInput" placeholder="Title" v-model="Title"/>
            </div>
            <div v-if="step === 0">
            ...
            <div v-if="step === 1">
                ...
                <input type="text" id="categoryInput" name="categoryInput" placeholder="Category" v-model="Category"/>
            </div>
            <div v-if="step === 1">
                <label for="Tags" > Tags     </label>
                <textarea v-model="Tags" placeholder="Please write your tags with commas. For Example: racism,government,issues"/>
            </div>
            <div v-if="step === 2">
...
.CreateContainer input[type=text], input[type=password]  {
    @apply border-b-4 border-gray-300 text-xl w-112;
}

.CreateContainer input[type=text]:focus-within , input[type=password]:focus-within {
    @apply text-blue-400 border-blue-400;
}

.CreateContainer input[type=text]:focus::placeholder, input[type=password]:focus::placeholder {
    @apply text-blue-400;
}
...
.CreateContainer input[type=submit], input[type=button] {
    @apply p-4 w-80 text-2xl bg-blue-400 text-white rounded-xl transition;
}

.CreateContainer input[type=submit]:hover, input[type=button]:hover {
    @apply bg-blue-500;
}
